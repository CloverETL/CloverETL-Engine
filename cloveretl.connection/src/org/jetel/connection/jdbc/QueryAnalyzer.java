	...
	public String getUpdateDeleteQuery() {
		...
		while(mappingMatcher.find()){
			if (!StringUtils.isEmpty(mappingMatcher.group(3))) {//found dbField = $field
				dbCloverFieldMap.add(new String[]{mappingMatcher.group(2), mappingMatcher.group(3)});
//				dbCloverFieldMap.put(mappingMatcher.group(2), mappingMatcher.group(3));
			}else{//found dbField = ?
				dbCloverFieldMap.add(new String[]{mappingMatcher.group(2), null});
//				dbCloverFieldMap.put(mappingMatcher.group(2), null);
	...
	public String getSelectQuery(){
		...
		if (mappingMatcher.find()){
			do {
				cloverDbFieldMap.add(new String[]{mappingMatcher.group(2), mappingMatcher.group(3)});				
			...
			if (fieldList.length != 1 || !fieldList[0].equals("*")) {//not: select * from
				for (int i = 0; i < fieldList.length; i++) {
					cloverDbFieldMap.add(new String[] { null,	fieldList[i].trim() });
				}
			}
		}
		mappingMatcher.appendTail(sb);
