 ...
 * @created 4.4.2012
 */
public class DeleteFiles extends AbstractFileOperation<DeleteResult> {
    ...
    public DeleteFiles(String id, TransformationGraph graph) {
		super(id, graph);
	}
	...
	@Override
	protected DeleteResult executeOperation() throws InterruptedException {
		String message = MessageFormat.format(FileOperationComponentMessages.getString("DeleteFiles.deleting"), target); //$NON-NLS-1$
		tokenTracker.logMessage(inputRecord, Level.INFO, message, null);
    ...
    public static Node fromXML(TransformationGraph transformationGraph, Element xmlElement) throws XMLConfigurationException {
        ...
        try {
            ...
            deleteFiles = new DeleteFiles(componentAttributes.getString(XML_ID_ATTRIBUTE), transformationGraph);

            deleteFiles.setTarget(componentAttributes.getStringEx(XML_TARGET_ATTRIBUTE, null, RefResFlag.SPEC_CHARACTERS_OFF));
            if (componentAttributes.exists(XML_RECURSIVE_ATTRIBUTE)) {
            	deleteFiles.setRecursive(componentAttributes.getBoolean(XML_RECURSIVE_ATTRIBUTE, false));
            }
            AbstractFileOperation.fromXML(deleteFiles, componentAttributes);
