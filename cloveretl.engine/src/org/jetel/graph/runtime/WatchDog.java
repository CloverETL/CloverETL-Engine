...
//    private volatile boolean runIt;
	...
	public synchronized void graphStarted() {
		...
		if (provideJMX) {
			...
			logger.info("WatchDog sending JMX message GRAPH_STARTED with runId " + getGraphRuntimeContext().getRunId());
	...
	public synchronized void phaseFinished() {
		...
		if (provideJMX) {
			...
			logger.info("WatchDog sending JMX message PHASE_FINISHED with runId " + getGraphRuntimeContext().getRunId());
